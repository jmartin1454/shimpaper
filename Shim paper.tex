...
relaxation~\cite{bib:pignol-and-roccia-and-some-other-papers-by-golub-pendlebury-others-with-at-least-one-that-talks-about-neutron-precession}.
...
Ref.~\cite{bib:earliercitedworkinAbel2019,bib:Abel2019},
\begin{equation}
  \frac{1}{T_{2,\rm mag}}=
...
results and describe a path forward to complete the basic design.
...
Fig.~\ref{fig:geometry}, it is a $3\times 3\times 3$ ``coil cube''.
...
straight-line segment as discussed earlier.
...
$\vec{B}=0$ in all space.  This mode appears in the singular value decomposition as
\begin{equation}
  S_{cc}=0,
\end{equation}
...
\subsection{Matrix methods}\label{sec:matrixmethods}


A software tool [\url{https://github.com/jmartin1454/squares}] was
written to study the ability of different shim coil arrangements to
correct arbitrary fields within the central region.  In the beginning
of the design studies, coils in the simulation would be wound as
regular square coils on the six surfaces of a cube.  An example
geometry is shown in Fig.~\ref{fig:3x3}.  The magnetic field produced
by the coils would be sensed at test points within the central region
of interest.  Initial studies were conducted in free space for ease of
calculation.  In Section~\ref{sec:where}, modifications to the code to
use COMSOL-calculated fields are discussed.
...
The target field $B_{\mathrm{target}}$ may represent either a correction field used to reduce inhomogeneity metrics such as $\Delta B_z$ and $\sigma(B_z)$, or a specific harmonic mode $G_{\ell,m}$ used for systematic studies. Once the set currents are obtained, the achieved field is evaluated using the same ROI metrics and compared with the mapping measurements described in Section~\ref{sec7}.
  ...
  The
design tool performs the following steps in the calculation:
\begin{enumerate}

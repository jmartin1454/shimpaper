...
\begin{figure}[tbp]
  ...
  \caption{Calculated neutron visibility $\alpha$ after $T=180$~s as a
    ...
    gravitationally enhanced depolarization and intrinsic
    depolarization. The curves are obtained from the first-order gradient model
using Eqs.~(\ref{eq:alphagrav}) and~(\ref{eq:alphamag})
with parameters
$T = 180~\mathrm{s}$,
$\mathrm{Var}(\bar{z}) = 0.18~\mathrm{cm}^{2}$,
$v = 3.0~\mathrm{m\,s^{-1}}$,
$H = 0.12~\mathrm{m}$,
and $G_{1,1}=G_{1,-1}=0$.
...
was based on similar studies performed in\cite{bib:klassen-msc}. In
that case, the magnetic shielding of old ILL nEDM experiment, then at
PSI, was known to possess a native 3 nT/m gradient  \cite{pendlebury2004}, 
and this was used as a benchmark.
...
range~\cite{bib:msr_rsi}, with a significant probability of
improvement with more careful degaussing.  In the new n2EDM shield,
the PSI collaboration also demonstrated a magnetic-field
non-uniformity of $\sigma(B_z)=27$~pT over a cylindrical volume of
radius 40~cm and height 30~cm~\cite{bib:n2edm-uniformity2025}.  The
homogeneity improved to 300 pT in 1.4 m$^3$ after more detailed
degaussing studies \cite{bib:Segarra2024Degaussing} and the degaussing
time was substantially reduced.  The homogeneity in the PanEDM
experiment shield was demonstrated to be to be less than
300~pT/m~\cite{bib:Altarev2014MSR}.  Other similarly sized MSRs have
demonstrated even better homogeneity, with residual fields below
130~pT in a $0.5\times0.5\times0.5$~m$^3$
volume~\cite{bib:Sun2021Limits}.
...
The worst result seen in Table~\ref{tab:trials}, indicated by the
boxed value, seems to be driven in part by a particularly pathological
$\partial B_z/\partial x$.  While it does not meet the requirement set
in Ref.~\cite{bib:jeffnote} that $\Delta B_z<140$~pT, recall that this
...
\subsection{Selection of coil paths}

The final coil paths for the prototype shim coil system are shown in
Fig.~\ref{fig:modeste}.
\begin{figure}
\centering
\includegraphics[width=\textwidth]{MSR_E-W-wall.png}
\caption{Final coil paths for the prototype shim coil system, shown in
  simulation.  The figure also indicates the numbering of the coils.
  The East wall has coils 0 through 8.  The same 27 test points as
  were used in Fig.~\ref{fig:geometry} were used to sample the
  magnetic field produced by the coils.}
\label{fig:modeste}
\end{figure}
The final coil paths were chosen to mimic the originally satisfactory
geometry of Fig.~\ref{fig:geometry}, with the following changes:
\begin{itemize}
\item We decided to relocate the coils to the inner wall of the MSR.
  This would easy mounting during the prototyping and testing phase.
  The walls are located at a distance of 1.1~m from the centre of the
  MSR, as can be seen in Fig.~\ref{fig:modeste}.
\item Current paths should not conflict with feedthrough holes in the
  MSR walls, which are used for access.  This necessitated the
  somewhat different rectangular shapes.
\item The coils for each wall should fit within a $4'\times 4'$ area.
  This was done for ease of our construction method, which relied on a
  coroplast backing, as will be described in
  Section~\ref{sec:whatever}.
\end{itemize}
Ultimately, we plan to relocate the coils to be inside the $B_0$ coil
for the nEDM experiment, which is still being constructed.  The coils
are likely to be moved inward by about 15~cm compared to where they
were located for these measurements.

As the final coil paths were being decided, we also repeatedly checked
the ability of the shim coil system to reduce magnetic inhomogeneities
satisfactorily, as in Table~\ref{tab:trials}.  As before, the
calculations were still being done in free space.  In general, it was
found that moving the coils outward to the MSR wall location had a
significant improvement on the homogeneity in the central region, with
only a slight increase in the required currents.  The results of the
same homogeneity tests is shown in Table~\ref{tab:modeste}.

It had been found in earlier studies that using almost any coil
configuration with sufficient granularity would meet the homogeneity
requirement.  The exact selection of the coil paths does not seem to
be a critical parameter in the design.  This was a somewhat surprising
result, but allowed us to freely relocate coil paths as necessary to
avoid conflicts.  Our studies indicate that this method of magnetic
field control and shimming has a large amount of flexibility in terms
...
\begin{table}
  \begin{center}
    \begin{tabular}{ccccccc}
      ...
      goal & & 140 & & 40 & & 0.1 \\
    \end{tabular}
    \caption{FIX THIS TABLE IT IS STILL THE SAME AS THE PREVIOUS GET
      FROM MODESTE Trials using the final coil geometry, testing their
      ability to counteract various applied
      fields.\label{tab:modeste}}
  \end{center}
\end{table}

We have now completed COMSOL simulations which include the effects of
the highly permeable MSR walls.  The results also indicate
satisfactory homogeneity for the various field shapes assumed in the
study.  The COMSOL simulations will be described in
Section~\ref{sec:something}, and compared with data.

\section{Design and construction of current source}

Using the free space simulation described above, and the same assumed
magnetic inhomogeneities, of varying spatial dependences but always
3~nT max-min across the central 1~m$^3$ volume, the currents required
for the coils were surveyed.  Assuming one turn for each coil, 
...
better than the stability requirement for shim-coil operation (EXPLAIN
THE REQUIREMENT SOMEWHERE). A scale-model coil experiment also
demonstrated that the supply can reproduce calculated current patterns
accurately. In the present work, this current supply was used to apply
the matrix-calculated setpoints for individual-coil tests and
harmonic-mode measurements.
...
and easy to cut and handle. The coil geometry was drawn directly on
the sheet using measurement tools and a marker.  Grooves were cut into
the coroplast surface to hold the wire placement.  Grooves across the
ribs were made using a thin circular-saw blade, while grooves along
the ribs were made using a knife.

Each square coil was wound using 30~AWG magnet wire with three turns
per coil.  Three turns were used because we had temporarily planned to
use a simpler, less homogeneous $B_0$ coil, and implementing the three
turns would have allowed us to correct the somewhat worse
inhomogeneity of that coil.  Although the two coils were never used
together, this was the idea of the decision.

For each panel (wall), lead wires from each coil were twisted together
and routed back to a common exit point, simplifying cable
organization. To avoid blocking the feedthrough holes in the walls of
the MSR, cutouts were made in the coroplast sheets
(Fig.~\ref{fig:msr_prototype_install}).

\subsection{Installation inside the MSR and cabling}

The six prototype panels were secured at the centres of the MSR
interior walls using a combination of double-sided tape, and
mechanical fasteners when mount points where available.  Tape was used
to allow easy removal of prototype panels at a later date.  For each
face, nine coils require eighteen conductors (two leads per
coil). Ethernet cables were used as convenient multi-conductor
bundles, with three cables per face and all cables routed through the
MSR extrusions to a centralized location for easier connection to the
current source.
...
\begin{figure}[tbp]
  ...
  \caption{Prototype square shim coils installed inside the TUCAN
    ...
    the side-wall, ceiling, and far-end panels are visible.  Floor
    coils are located underneath the panelled floor, while the
    door-side panels are outside the field of view because the door
    has been pulled back to allow access (that panel is mounted on the
    door itself).}
...
The prototype shim-coil system was characterized by measuring the
magnetic field along the $x$ direction through the MSR. A three-axis
fluxgate magnetometer was used to measure $B_x$, $B_y$, and $B_z$ at
each scan position. Measurements were performed with both positive and
negative applied coil currents. The magnetic field produced by the
shim coils was determined from

\begin{equation}
B_{\mathrm{coil}}
=
\frac{B(+I)-B(-I)}{2},
\label{eq:coil_difference}
\end{equation}

which reduces the contribution from the background field inside the
MSR and isolates the magnetic field that changes sign with the applied
coil current.

Measurements were first performed for each shim coil individually.
The system was then operated using current patterns calculated to
generate selected harmonic-gradient fields. These measurements provide
two complementary tests of the system. The individual-coil
measurements test the response of the physical coils relative to the
COMSOL model, while the harmonic-mode measurements test whether the
responses of many coils can be combined to generate the desired
spatial field patterns.

Additional measurements were performed along the $y$ direction through
the MSR door. These measurements provide a second scan geometry and
allow the coordinate transformation used for the fluxgate measurements
to be examined.


% ==========================================================
\subsection{Individual-coil measurements}
\label{sec:individual_coils}
% ==========================================================

Each of the 54 shim coils was mapped individually using currents of
$+40$~mA and $-40$~mA. The measured field was compared with the field
calculated by COMSOL for the same coil geometry.
Figure~\ref{fig:east_wall} shows the nine coils on the east wall as a
representative example. The same procedure was used for the coils on
the other five faces of the system.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{east_wall_fit.pdf}
    \caption{Measured magnetic-field components for the nine shim coils
    on the east wall, compared with the corresponding COMSOL
    calculations. The points show the measured $B_x$, $B_y$, and $B_z$
    components. The solid curves show the nominal COMSOL calculations,
    while the dashed curves show the COMSOL fields after applying the
    fitted scale factor $s$ for each individual coil.}
    \label{fig:east_wall}
\end{figure}

The measured profiles reproduce the principal spatial features
predicted by COMSOL. The relative signs and spatial dependences of the
three magnetic-field components are also reproduced. The main
difference is in the overall field amplitude. For this reason, a
multiplicative scale factor was fitted between the measured and COMSOL
fields for each coil.

For the east-wall coils shown in Fig.~\ref{fig:east_wall}, the fitted
scale factors are approximately 0.85--0.96. Thus, for these coils, the
measured field amplitude is generally slightly smaller than the
nominal COMSOL prediction while the overall spatial structure remains
similar.

The fitted scale factors for all 54 shim coils are shown in
Fig.~\ref{fig:coil_scale_factors}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\linewidth]{coil_scale_factors.pdf}
    \caption{Fitted COMSOL scale factor for each of the 54 shim coils.
    A value of one corresponds to the nominal COMSOL response. Most of
    the coils form a common distribution near 0.9, while a small number
    exhibit substantially different responses.}
    \label{fig:coil_scale_factors}
\end{figure}

Most of the coils exhibit a similar response, with fitted scale factors
clustered near 0.9, while a smaller number show larger deviations. The
scale factor is used here as an empirical calibration of the installed
coil response. A deviation from one does not, by itself, identify the
physical origin of the difference.

These measurements become important when several coils are operated
simultaneously. A common amplitude difference can be described by one
overall normalization factor. However, coil-to-coil differences can
modify the shape of a magnetic field generated from a combination of
many coils. This becomes particularly important for the higher-order
harmonic modes discussed below.The magnetic field in the measurement region can be represented as a superposition of harmonic-gradient modes $G_{\ell,m}$, following the convention used in nEDM magnetic-field analyses~\cite{Abel2019}.
Here, $\ell$ denotes the order of the spatial variation and $m$
distinguishes the different field symmetries within a given order.
Generating selected $G_{\ell,m}$ modes therefore provides a useful
test of whether the fields from many individual shim coils can be
combined to reproduce more complicated and well-defined spatial field
structures. In the following measurements, the required coil-current
setpoints were obtained from the response matrix for the corresponding
target harmonic field.


% ==========================================================
\subsection{Generation of the $G_{1,0}$ mode}
\label{sec:g10}
% ==========================================================

As a first test using all of the coils together, current setpoints were
calculated to produce the $G_{1,0}$ harmonic-gradient mode. The COMSOL
response matrix was used to determine the required current in each of
the 54 coils. The calculated currents were then applied to the
installed shim-coil system and the magnetic field was measured along
the same $x$-scan direction.

Figure~\ref{fig:g10} compares the measured magnetic field with the
COMSOL prediction and with the ideal $G_{1,0}$ harmonic target.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{G10_paper_target.pdf}
    \caption{Measured magnetic-field components for the $G_{1,0}$
    current pattern compared with the COMSOL prediction and the ideal
    harmonic target. The coil-current setpoints were obtained from the
    COMSOL response matrix. The dashed curve shows the COMSOL
    prediction after applying a single fitted overall scale factor.}
    \label{fig:g10}
\end{figure}

The measured field follows the spatial dependence predicted by COMSOL.
The dominant component varies approximately linearly through the
central part of the scan. Toward the ends of the scan, the field
generated by the finite coil array departs from the ideal straight-line
harmonic target.

A small overall difference in amplitude is also observed between the
measurement and the COMSOL calculation. Fitting a single scale factor
to the measured and calculated field components gives

\begin{equation}
s = 0.937.
\label{eq:g10_scale_factor}
\end{equation}

Thus, for this mode, the measured magnetic-field amplitude is
approximately 94\% of the nominal COMSOL prediction. This value is
consistent with the approximately 0.9 response observed for most of
the individual shim coils.

The $G_{1,0}$ measurement therefore provides a first test of the
complete multi-coil system. It demonstrates that the response-matrix
method produces the expected spatial field structure when the
contributions from all 54 coils are combined, while the remaining
overall amplitude difference can largely be described by a single
normalization factor.


% ==========================================================
\subsection{Coil calibration and the $G_{1,2}$ mode}
\label{sec:g12}
% ==========================================================

Higher-order harmonic modes provide a more sensitive test of the
system because their field shapes depend on the relative contributions
from many different coils. One example is the $G_{1,2}$ mode.

The nominal COMSOL calculation reproduces the principal $G_{1,2}$
field structure, but the measured field contains an additional local
feature, most clearly visible in $B_z$ on the negative-$x$ side of the
scan. This feature is not reproduced when every coil is assumed to
have exactly its nominal COMSOL response.

The individual-coil measurements provide a means of testing the origin
of this difference. The expected field was recalculated after including
the measured scale factor for each individual coil. The result is shown
in Fig.~\ref{fig:g12_calibration}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]
    {G12_calibration_comparison.pdf}
    \caption{Measured $G_{1,2}$ magnetic field compared with the COMSOL
    prediction. (a) Original calculation using the nominal COMSOL
    response of each coil. (b) Calculation after including the measured
    individual-coil calibration factors. The ideal harmonic target is
    shown by the dotted curve.}
    \label{fig:g12_calibration}
\end{figure}

Including the individual-coil calibration factors produces a clear
improvement in the calculated field. In particular, the calculation
reproduces more closely the local $B_z$ feature observed in the
measurement. The sign, position, and approximate magnitude of this
feature are recovered when the different measured responses of the
individual coils are included.

The improvement is also reflected in the residuals. For $B_z$, the
RMS residual over the full scan decreases from approximately
5.562~nT to 1.480~nT after applying the individual-coil calibration.
When the three field components are considered in the central region
of interest, the RMS residual decreases from approximately 2.195~nT
to 0.556~nT.

% Check these RMS values against the final calibration output
% before manuscript submission.

The calibration does not remove every difference between measurement
and simulation. Nevertheless, the comparison demonstrates that the
unusual feature observed in the $G_{1,2}$ measurement can be largely
reproduced when the measured coil-to-coil response differences are
included. It also shows why a single global normalization is not
necessarily sufficient for accurate prediction of higher-order modes.


% ==========================================================
\subsection{Response-matrix dependence of the $G_{2,1}$ mode}
\label{sec:g21}
% ==========================================================

A different behaviour was observed for the $G_{2,1}$ mode. The ideal
$G_{2,1}$ field is symmetric about the centre of the scan, whereas the
measurement obtained using the COMSOL-derived current setpoints shows
a clear left--right asymmetry.

To investigate whether this behaviour is related to the current
solution, two sets of coil currents were compared. One set was
calculated using the free-space response matrix, while the second was
calculated using the COMSOL response matrix. The results are shown in
Fig.~\ref{fig:g21_comparison}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]
    {G21_freespace_vs_comsol.pdf}
    \caption{Measured $G_{2,1}$ magnetic field obtained using
    (a) free-space-derived coil-current setpoints and
    (b) COMSOL-derived coil-current setpoints. The measured magnetic
    field is compared with the corresponding calculated field and with
    the ideal harmonic target.}
    \label{fig:g21_comparison}
\end{figure}

The currents obtained from the free-space response matrix produce a
more symmetric measured profile, although the resulting field does not
reproduce the target amplitude and shape as closely. The
COMSOL-derived currents provide better overall agreement with the
target through much of the scan, but a clear left--right asymmetry
remains.

This behaviour differs from that observed for $G_{1,2}$. In the
$G_{1,2}$ case, the principal discrepancy can be largely reproduced by
including the measured responses of the individual coils. For
$G_{2,1}$, the asymmetry is associated with the current solution
obtained using the COMSOL response matrix.

The origin of the $G_{2,1}$ asymmetry has not yet been identified.
Possible contributions include details of the response matrix, COMSOL
mesh, model geometry, and magnetic boundary conditions. We therefore
do not interpret this result as a simple failure of the shim-coil
hardware or as a straightforward individual-coil calibration effect.
Instead, it provides a useful diagnostic for continued improvement of
the field model and response matrix.


% ==========================================================
\subsection{Third-order harmonic modes}
\label{sec:third_order}
% ==========================================================

The harmonic-mode measurements were extended to selected third-order
modes in order to test the ability of the shim-coil system to generate
more complicated spatial magnetic-field structures. Two representative
examples, $G_{3,0}$ and $G_{3,-2}$, are shown in
Fig.~\ref{fig:third_order}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]
    {G30_G3-2_third_order.pdf}
    \caption{Measured magnetic-field components for selected
    third-order harmonic modes compared with the corresponding COMSOL
    calculations and ideal harmonic targets:
    (a) $G_{3,0}$ and (b) $G_{3,-2}$.
    The points show the measured $B_x$, $B_y$, and $B_z$ components,
    the solid curves show the COMSOL calculations, and the dotted
    curves show the ideal harmonic targets.}
    \label{fig:third_order}
\end{figure}

For the $G_{3,0}$ mode, the measured magnetic-field components follow
the principal spatial structure predicted by COMSOL. Differences
between the field generated by the finite coil array and the ideal
harmonic target become more pronounced toward the ends of the scan.
The measured points nevertheless follow the corresponding COMSOL field
structure over the measured range.

The $G_{3,-2}$ mode gives a different spatial dependence and a
different combination of magnetic-field components from the
$G_{3,0}$ mode. The measured components again reproduce the principal
features predicted by the COMSOL calculation.

These measurements extend the harmonic-mode tests to more complicated
third-order field structures. They demonstrate that the response-matrix
method can be used to generate a range of spatial magnetic-field
patterns with the installed shim-coil array. Exact agreement with the
ideal harmonic expressions over the complete scan range is not
expected because the magnetic field is generated by a finite coil
system.


% ==========================================================
\subsection{Door-scan measurements and coordinate rotation}
\label{sec:door_scan}
% ==========================================================

Additional measurements were performed along the $y$ direction through
the MSR door. For this scan geometry, comparison of the measured vector
field with the COMSOL calculation required an additional rotation of
the measured $B_x$ and $B_z$ components.

A rotation of $5^\circ$ in the $x$--$z$ plane was applied according to

\begin{equation}
B_x'
=
B_x\cos\theta + B_z\sin\theta ,
\label{eq:door_rotation_bx}
\end{equation}
...
B_z'
=
-B_x\sin\theta + B_z\cos\theta ,
\label{eq:door_rotation_bz}
\end{equation}
...
\begin{equation}
\theta = 5^\circ .
\label{eq:door_rotation_angle}
\end{equation}

The $B_y$ component is unchanged by this additional rotation.

The effect of the coordinate correction is shown for coil 13 in
Fig.~\ref{fig:door_rotation13}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]
    {coil_13_door_rotation_comparison.pdf}
    \caption{Door-side $y$ scan for coil 13 compared with the COMSOL
    calculation. (a) Measured magnetic-field components before
    applying the $5^\circ$ rotation of the $B_x$ and $B_z$ components.
    (b) The same measurement after applying the coordinate rotation.
    The RMS value quantifies the overall three-component residual
    relative to the COMSOL calculation.}
    \label{fig:door_rotation13}
\end{figure}

Before the coordinate correction, a nonzero $B_z$ component is visible
in the measured data while the corresponding COMSOL component remains
close to zero over most of the scan. After applying the $5^\circ$
rotation, the apparent $B_z$ contribution is substantially reduced and
the measured component structure agrees more closely with the COMSOL
calculation.

The same comparison for coil 40 is shown in
Fig.~\ref{fig:door_rotation40}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]
    {coil_40_door_rotation_comparison.pdf}
    \caption{Door-side $y$ scan for coil 40 compared with the COMSOL
    calculation. (a) Measured magnetic-field components before
    applying the $5^\circ$ coordinate rotation.
    (b) The same measurement after applying the rotation of the
    $B_x$ and $B_z$ components.}
    \label{fig:door_rotation40}
\end{figure}

A similar reduction of the apparent $B_z$ component is observed for
coil 40 after applying the coordinate rotation. The comparison
therefore shows that the effect is not restricted to a single coil.

The improvement obtained after the rotation is consistent with a small
relative angular offset between the fluxgate measurement axes and the
coordinate system used for the field calculation. Such an offset can
mix the measured $B_x$ and $B_z$ components. The door-scan comparison
therefore provides an additional check of the coordinate transformation
used when comparing the measured vector field with the COMSOL model.
% ==========================================================
\section{Discussion and conclusion}
\label{sec:discussion_conclusion}
% ==========================================================

A 54-coil square shim-coil system has been constructed and tested inside
the TUCAN magnetically shielded room. The measurements presented here
characterize the system at several levels: the response of individual
coils, the generation of harmonic field patterns using many coils
simultaneously, the effect of individual-coil calibration, and the
coordinate treatment required for vector-field measurements.

The individual-coil measurements show that the installed coils reproduce
the principal spatial field structures predicted by COMSOL. Most coils
have similar amplitude responses, with fitted scale factors clustered
near 0.9, while a smaller number show larger deviations. These
measurements therefore provide an experimental calibration of the
installed coil responses in addition to the nominal geometrical model.
The coil-to-coil variations are particularly relevant when several coils
are combined to generate higher-order field patterns.

The $G_{1,0}$ measurement provides a first test of the complete
multi-coil system. The measured field follows the spatial dependence
predicted by COMSOL, with an overall fitted scale factor of
$s=0.937$. This result shows that the response-matrix approach can be
used to calculate combinations of coil currents that generate the
expected first-order spatial field structure. The remaining overall
amplitude difference can largely be described by a single
normalization factor.

The higher-order harmonic measurements provide more sensitive tests of
the relative coil responses and of the response matrix. For the
$G_{1,2}$ mode, the local $B_z$ feature that is not reproduced by the
nominal COMSOL calculation is substantially better reproduced when the
experimentally determined individual-coil calibration factors are
included. The improvement is also quantitative: the full-scan $B_z$
RMS residual decreases from 5.562~nT to 1.480~nT, while the
three-component RMS residual in the central region decreases from
2.195~nT to 0.556~nT. These results demonstrate that differences in
the responses of individual coils can significantly affect the field
generated by a multi-coil current pattern and that a single global
normalization is not sufficient for all higher-order modes.

The $G_{2,1}$ mode exhibits a different behaviour. The
free-space-derived current solution produces a more symmetric measured
profile, whereas the COMSOL-derived current solution shows a clear
left--right asymmetry. The origin of this effect has not yet been
identified. The present measurements therefore do not justify
attributing the discrepancy to a particular component of the hardware
or model. Instead, the $G_{2,1}$ result provides a useful diagnostic
for further investigation of the COMSOL model, response-matrix
construction, numerical precision, geometry, and magnetic boundary
conditions.

Measurements of the $G_{3,0}$ and $G_{3,-2}$ modes extend the
characterization to third-order harmonic fields. The measured field
components reproduce the principal spatial features predicted by the
finite-coil COMSOL calculation. Differences from the ideal harmonic
targets become more apparent toward the ends of the scan, which is
consistent with the fact that the target represents an ideal harmonic
field whereas the experimental field is generated by a finite array of
coils with fixed geometry. The agreement in the main spatial features
shows that the array can generate field structures more complicated
than the lower-order modes.

The door-scan measurements provide an additional check of the
vector-field analysis and coordinate treatment. Applying a
$5^\circ$ rotation of the measured $B_x$ and $B_z$ components reduces
the apparent component mixing and improves agreement with the COMSOL
calculation for both representative coils. This behaviour is
consistent with a small relative angular offset between the fluxgate
measurement axes and the coordinate system used in the field model.

The characterization presented here is based primarily on
one-dimensional scans through the central region of the MSR,
supplemented by measurements through the door. These measurements
therefore validate important aspects of the prototype shim-coil system
and the response-matrix method, but they do not by themselves
demonstrate the final magnetic-field homogeneity throughout the full
EDM-cell volume.

The next stage of the work will be to integrate the shim-coil system
with the $B_0$ coil and determine the shim-current settings under the
magnetic-field conditions of the assembled experiment. Full-volume
field mapping will be required to characterize the residual field and
to determine the correction currents needed to satisfy the field
homogeneity requirements. Further refinement of the COMSOL response
matrix using the measured individual-coil responses, together with
additional field information from the mapper and potentially Cs
magnetometers, will support this optimization.

Overall, the measurements demonstrate that the prototype square
shim-coil array can generate controlled magnetic-field patterns inside
the TUCAN MSR and that the principal spatial behaviour is described
reasonably well by the COMSOL model. The results also show that
individual-coil calibration, continued validation of the response
matrix, and careful treatment of the measurement coordinate system are
important for accurate magnetic-field control. These studies provide
the experimental basis for the next stage of magnetic shimming in the
...
Magnetic field mapping of the shim-coil system was carried out along a
central scan axis (X????) using a three-axis fluxgate magnetometer.
The axes of the fluxgate magnetometer were aligned as well as possible
with the axes of the MSR (*** to a systematic precision of about
5$^\circ$ for each measurement***???).  The MSR was used to define the
coordinate system, with the centre of the MSR being located at the
origin $(x,y,z)=(0,0,0)$.  The probe was moved from $-99.5$~cm to
$+100.5$~cm in steps of 10~cm, where $z=0$ (EXPLAIN THE COORDINATE
SYSTEM I DO NOT UNDERSTAND WHY Z IS USED HERE) corresponds to the
center of the system. At each position, a measurement of the field was
made for each fluxgate axis. were taken for both positive and negative
applied current. EXPLAIN WHAT A POSITIVE CURRENT MEANS.  The shim-coil
field was then obtained from
\begin{equation}
...
Each of the 54 shim coils was mapped individually. The measurement
procedure described above was applied to each coil in turn using $\pm
40$~mA. Maps measured for all individual coils were in good agreement
with the simulated fields ***HOW GOOD?***. Figure~\ref{fig:singlecoil}
shows three representative examples ***NEED FIGURE FOR NINE***. In
each case, the measured $B_x$, $B_y$, and $B_z$ components reproduce
the main features of the corresponding COMSOL curves.
\begin{figure}
  \centering
  \includegraphics[width=0.95\linewidth]{east_wall_fit.pdf}

  \caption{Measured magnetic-field components $B_x$, $B_y$, and $B_z$
  along the central scan axis for the individual shim coils on the east
  wall (coils 0--8), compared with the corresponding COMSOL calculations.
  The solid curves show the COMSOL predictions, while the dashed curves
  show the COMSOL fields after applying the fitted overall scale factor
  for each coil. The fitted scale factor is indicated in each panel.}

  \label{fig:east_wall_coils}
\end{figure}
EXPLAIN COMSOL SIMULATION.

...
The prototype shim-coil system was tested by measuring the magnetic
field along the central $x$-scan axis of the MSR. A three-axis fluxgate
magnetometer was used to measure $B_x$, $B_y$, and $B_z$ at each scan
position. Measurements were made with both positive and negative coil
currents. The field produced by the shim coils was determined from
...
inside the MSR and isolates the field that changes sign with the applied
coil current.

The measurements were first made for each coil individually. The system
was then operated with current patterns calculated to generate selected
harmonic-gradient fields. These measurements provide two different
tests of the system. The individual-coil measurements test the response
of the physical coils relative to the COMSOL model, while the
harmonic-mode measurements test whether the responses of many coils can
be combined to generate the desired field shapes.
...
calculated by COMSOL for the same coil geometry. Figure~\ref{fig:east_wall}
shows the nine coils on the east wall as a representative example.
The same procedure was used for the coils on the other five faces of
the system.
...
near 0.9. A few coils show much larger deviations. In particular, coils
...
calculated to produce the $G_{1,0}$ harmonic-gradient mode. The
COMSOL response matrix was used to calculate the current required in
each of the 54 coils. The calculated currents were then applied to the
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{G12_calibration_comparison.pdf}
...
The improvement can also be seen from the residuals. For $B_z$, the RMS
residual over the full scan decreases from approximately 5.56~nT to
1.81~nT after applying the individual-coil calibration. When the three
field components are considered in the central region of interest, the
RMS residual decreases from approximately 2.37~nT to 0.71~nT.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{G21_freespace_vs_comsol.pdf}
    ...
    compared with the corresponding calculated field and with the ideal
    harmonic target.}
...
currents give better overall agreement with the target field, especially
through the central part of the scan, but a clear asymmetry remains.
...
matrix or the COMSOL model used to construct it as an area that requires
further investigation. Possible contributions include the numerical
precision of the response matrix, the COMSOL mesh, and details of the
model geometry and magnetic boundary conditions.
...
A 54-channel square shim-coil system has been constructed and tested
inside the TUCAN magnetically shielded room. The measurements presented
here test the system at three levels: the response of individual coils,
the generation of a simple first-order harmonic mode, and the generation
of more sensitive higher-order harmonic modes.

The individual-coil measurements show that the installed coils
reproduce the main spatial field shapes predicted by COMSOL. Most coils
have a similar amplitude response, with fitted scale factors near 0.9.
A smaller number of coils have significantly different responses. These
measurements therefore provide a direct calibration of the physical
coil system that is not contained in the nominal geometrical model.

The $G_{1,0}$ measurement shows that the contributions from all 54 coils
can be combined to generate the expected first-order field pattern.
The measured spatial dependence agrees well with the COMSOL
calculation, with an overall scale factor of 0.937. This gives confidence
that the response-matrix method can be used to calculate useful
multi-coil current patterns.

The higher-order measurements show why individual calibration and an
accurate response matrix are important. For $G_{1,2}$, a local feature
that was not reproduced by the nominal COMSOL calculation is largely
recovered after the measured response of each coil is included. The
result indicates that coil-to-coil response variations are an important
part of the $G_{1,2}$ discrepancy.

The $G_{2,1}$ result gives a different test. A pronounced asymmetry is
seen when the current setpoints are calculated from the COMSOL response
matrix, while the free-space-derived solution is more symmetric. This
suggests that the $G_{2,1}$ mode is particularly sensitive to the
response matrix used to determine the currents. Further work is needed
to determine the origin of this asymmetry and to improve the COMSOL
model and response-matrix construction.

The present measurements were made along one central scan axis and do
not by themselves demonstrate the final magnetic-field homogeneity
throughout the EDM-cell volume. The results should therefore be viewed
as a characterization and validation of the prototype shim-coil system,
rather than a final demonstration of the TUCAN shimming requirements.

The next step is to extend the measurements to the full central volume
of the MSR and construct a response matrix based on the calibrated
installed coil system. These measurements can then be used to optimize
the coil currents for the actual residual magnetic field after the
$B_0$ coil and the remaining experimental components are installed.
The final performance will be evaluated directly against the TUCAN
requirements on $\Delta B_z$, $\sigma(B_z)$, and the transverse field.

Overall, the measurements show that the square shim-coil array can
produce controlled magnetic-field patterns inside the TUCAN MSR and
that its response is described reasonably well by the COMSOL model.
They also identify the two main improvements required for accurate
field control: calibration of the individual coil responses and further
validation of the response matrix used to calculate multi-coil current
setpoints.

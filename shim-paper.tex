...
each scan position. Measurements were made with both positive and
negative coil currents. The field produced by the shim coils was
determined from
...
This subtraction reduces the contribution from the background field
inside the MSR and isolates the field that changes sign with the
applied coil current.

The measurements were first made for each coil individually. The
system was then operated with current patterns calculated to generate
selected harmonic-gradient fields. These measurements provide two
different tests of the system. The individual-coil measurements test
the response of the physical coils relative to the COMSOL model,
while the harmonic-mode measurements test whether the responses of
many coils can be combined to generate the desired field shapes.
...
also allow the effect of the coordinate transformation used for the
fluxgate measurements to be examined.
...
\begin{figure}[htbp]
    ...
    components. The solid curves are the original COMSOL calculations
    and the dashed curves show the COMSOL fields after applying the
    fitted scale factor for each coil.}
    \label{fig:east_wall}
\end{figure}
...
\begin{itemize}
\item The coils work.
\item Scale factors between .85 and .96
\end{itemize}


The measured profiles reproduce the main spatial features predicted by
COMSOL. The relative sizes and signs of the three field components also
follow the simulation. The main difference is in the overall field
amplitude. For this reason, a multiplicative scale factor was fitted
between the measured field and the COMSOL field for each coil.

For the east-wall coils shown in Fig.~\ref{fig:east_wall}, the fitted
scale factors are approximately 0.85--0.96. This means that the
measured field from these coils is generally slightly smaller than the
nominal COMSOL prediction, while the spatial shape remains similar.

The fitted scale factors for all 54 coils are shown in
Fig.~\ref{fig:coil_scale_factors}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{coil_scale_factors.pdf}
    \caption{Fitted scale factor for each of the 54 shim coils. A value
    of one corresponds to the nominal COMSOL response. Most coils have
    scale factors below one and form a common group near 0.9, while a
    small number of coils show substantially different responses.}
    \label{fig:coil_scale_factors}
\end{figure}

Most of the coils have a similar response, with scale factors clustered
near 0.9. A few coils show larger deviations. In particular, coils
9--11 and coil 49 have responses above the main distribution. The scale
factor is used here as an empirical calibration of the coil response.
A deviation from one does not by itself identify its physical origin.

These measurements are important when several coils are used
simultaneously. A common scale difference can be corrected with one
overall normalization. However, differences from coil to coil can
change the shape of a field produced from a combination of many coils.
This becomes important for the higher-order harmonic modes discussed
below.
...
response matrix was used to calculate the current required in each of
the 54 coils. The calculated currents were then applied to the
installed shim-coil system and the field was measured along the same
$x$-scan direction.

Figure~\ref{fig:g10} compares the measured field with the COMSOL
prediction and with the ideal $G_{1,0}$ target field.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\linewidth]{G10_paper_target.pdf}
    \caption{Measured field for the $G_{1,0}$ current pattern compared
    with the COMSOL prediction and the ideal harmonic target. The
    current setpoints were obtained from the COMSOL response matrix.}
    \label{fig:g10}
\end{figure}
...
\begin{itemize}
\item show fit
\item fit generally agrees with scale factors
\end{itemize}

The measured field follows the spatial dependence predicted by COMSOL.
The dominant component changes approximately linearly through the
central part of the scan. The COMSOL field departs from the ideal
straight-line target near the ends of the scan. This is expected because
the finite coil system cannot reproduce the ideal harmonic field exactly
over an unlimited volume.

There is also a small overall difference in amplitude between the
measurement and the COMSOL calculation. Fitting one scale factor gives

\begin{equation}
s = 0.937,
\end{equation}

where the COMSOL field is multiplied by $s$ for comparison with the
measurement. Thus, for this mode, the measured field amplitude is about
94\% of the COMSOL prediction. This value is consistent with the
approximately 0.9 response observed for most of the individual coils.

The $G_{1,0}$ measurement therefore provides a useful first check of the
complete system. It shows that the response matrix gives the expected
field shape when the contributions from all 54 coils are combined. The
remaining amplitude difference can largely be described by one overall
scale factor.
...
Higher-order modes provide a more sensitive test because their field
shapes depend on the relative contributions from many different coils.
One example is the $G_{1,2}$ mode.

The original COMSOL calculation reproduces the main $G_{1,2}$ field
shape, but the measured field contains an additional local feature,
most clearly visible in $B_z$ on the negative-$x$ side of the scan.
This feature is not reproduced by the calculation when all coils are
assumed to have their nominal COMSOL response.

The individual-coil measurements provide a way to test the origin of
this difference. The expected field was recalculated using the measured
scale factor for each individual coil. The result is shown in
Fig.~\ref{fig:g12_calibration}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]
    {G12_calibration_comparison.pdf}
    \caption{Measured $G_{1,2}$ field compared with the COMSOL
    prediction. (a) Original prediction using the nominal COMSOL
    response of each coil. (b) Prediction after including the measured
...
improvement. In particular, the calculation now reproduces much more
closely the local $B_z$ feature seen in the measurement. The sign,
position, and approximate size of this feature are recovered when the
different measured responses of the coils are included.

The improvement can also be seen from the residuals. For $B_z$, the
RMS residual over the full scan decreases from approximately
5.56~nT to 1.81~nT after applying the individual-coil calibration.
When the three field components are considered in the central region
of interest, the RMS residual decreases from approximately 2.37~nT
to 0.71~nT.

The calibration does not remove every difference between measurement
and simulation. However, the comparison shows that the unusual feature
in the $G_{1,2}$ measurement is largely explained by the different
responses of the individual coils. This result also shows why a single
global normalization is not sufficient for accurate prediction of all
higher-order modes. For these modes, the response of each coil should
...
$G_{2,1}$ field is symmetric about the centre of the scan, while the
measurement made using the COMSOL-derived current setpoints shows a
clear left--right asymmetry.

To investigate whether this behaviour was produced by the current
solution, two sets of coil currents were compared. One set was
calculated using the free-space response matrix, and the other was
...
\begin{figure}[htbp]
    ...
    \caption{Measured $G_{2,1}$ field obtained using
    (a) free-space-derived coil-current setpoints and
    (b) COMSOL-derived coil-current setpoints. The measured field is
    compared with the corresponding calculated field and with the
    ideal harmonic target.}
    \label{fig:g21_comparison}
\end{figure}

The currents obtained from the free-space model produce a more
symmetric measured profile, although the resulting field does not
reproduce the target amplitude and shape as well. The COMSOL-derived
currents give better overall agreement with the target field,
especially through the central part of the scan, but a clear asymmetry
remains.

This comparison is different from the $G_{1,2}$ case. For $G_{1,2}$,
the main discrepancy can be largely reproduced by including the
measured response of each coil. For $G_{2,1}$, the asymmetry appears
when the COMSOL response matrix is used to determine the current
setpoints. The present measurements therefore point to the response
matrix or the COMSOL model used to construct it as an area that
requires further investigation. Possible contributions include the
numerical precision of the response matrix, the COMSOL mesh, and
details of the model geometry and magnetic boundary conditions.

At present, the origin of the $G_{2,1}$ asymmetry has not been
identified. We therefore do not interpret this result as a failure of
the shim-coil hardware or as a simple coil-calibration effect. Instead,
it provides a useful diagnostic for improving the field model and the
response matrix used to calculate the coil currents.
...
modes to test the ability of the shim-coil array to generate more
complicated spatial field structures. Two examples, $G_{3,0}$ and
$G_{3,-2}$, are shown here.

Figure~\ref{fig:g30} shows the measured field for the $G_{3,0}$ mode
together with the COMSOL calculation and the ideal harmonic target.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\linewidth]
    {G30_paper_target.pdf}
    \caption{Measured magnetic-field components for the $G_{3,0}$
    current pattern compared with the COMSOL calculation and the
    ideal harmonic target.}
    \label{fig:g30}
\end{figure}

The measured field follows the main spatial structure predicted by
COMSOL. Differences between the field generated by the finite coil
array and the ideal harmonic target become more pronounced toward the
ends of the scan. The measured points nevertheless follow the
corresponding COMSOL field structure over the measured range.

A second third-order example, the $G_{3,-2}$ mode, is shown in
Fig.~\ref{fig:g3m2}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\linewidth]
    {G3-2_paper_target.pdf}
    \caption{Measured magnetic-field components for the $G_{3,-2}$
    current pattern compared with the COMSOL calculation and the
    ideal harmonic target.}
    \label{fig:g3m2}
\end{figure}

The $G_{3,-2}$ mode gives a different spatial dependence and a different
combination of field components from the $G_{3,0}$ mode. The measured
components again reproduce the main features predicted by the COMSOL
calculation.

These measurements extend the harmonic-mode tests to more complicated
third-order field structures. They show that the response-matrix method
can be used to produce a range of spatial field patterns with the
installed shim-coil array. The comparison is primarily between the
measured field and the field generated by the finite coil system; exact
agreement with the ideal harmonic expression over the complete scan
...
Additional measurements were performed along the $y$ direction through
the MSR door. In this scan geometry, comparison of the measured vector
field with the COMSOL field required an additional rotation of the
measured $B_x$ and $B_z$ components.

A rotation of $5^\circ$ in the $x$--$z$ plane was applied in the
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{coil_13_door_rotation_comparison.pdf}
    \caption{Door-side $y$ scan for coil 13 compared with the COMSOL
    calculation. (a) Measured field components before applying the
    $5^\circ$ rotation of the $B_x$ and $B_z$ components.
...
in the measured data while the corresponding COMSOL component is close
to zero over most of the scan. After applying the $5^\circ$ rotation,
the apparent $B_z$ contribution is substantially reduced and the
measured component structure agrees more closely with the COMSOL
calculation.

The same comparison for coil 14 is shown in
Fig.~\ref{fig:door_rotation14}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]
    {coil_14_door_rotation_comparison.pdf}
    \caption{Door-side $y$ scan for coil 14 compared with the COMSOL
    calculation. (a) Measured field components before applying the
    $5^\circ$ coordinate rotation. (b) The same measurement after
    applying the rotation of the $B_x$ and $B_z$ components.}
    \label{fig:door_rotation14}
\end{figure}

A similar reduction of the apparent $B_z$ component is observed for
coil 14 after the coordinate rotation. The comparison therefore shows
that the treatment is not limited to a single coil.
...
coordinate system used for the field calculation. A small angular
offset can mix two measured field components. The door-scan comparison
therefore provides a useful check of the coordinate transformation used
when comparing the measured vector field with the COMSOL calculation.
...
A 54-coil square shim-coil system has been constructed and tested
inside the TUCAN magnetically shielded room. The measurements presented
here characterize the system at several levels: the response of
individual coils, the generation of harmonic field patterns using many
coils simultaneously, the effect of individual-coil calibration, and
the coordinate treatment required for vector-field measurements.

The individual-coil measurements show that the installed coils
reproduce the main spatial field shapes predicted by COMSOL. Most
coils have similar amplitude responses, with fitted scale factors
clustered near 0.9, while a smaller number show larger deviations.
These measurements provide an experimental calibration of the
...
predicted by COMSOL, with an overall fitted scale factor of 0.937.
This result shows that the response-matrix approach can be used to
calculate combinations of coil currents that generate the expected
first-order field pattern.

The higher-order measurements provide more sensitive tests of the
relative coil responses and of the response matrix. For the $G_{1,2}$
mode, the local $B_z$ feature that is not reproduced by the nominal
COMSOL calculation is largely recovered when the measured
individual-coil calibration factors are included. This shows the
importance of accounting for coil-to-coil response differences when
many coils are combined.

The $G_{2,1}$ mode gives a different result. The free-space-derived
current solution produces a more symmetric measured profile, while the
COMSOL-derived current solution shows a clear left--right asymmetry.
The origin of this effect has not yet been identified. The comparison
indicates that this mode is particularly sensitive to the response
matrix used to determine the coil currents and motivates further
investigation of the COMSOL model and response-matrix construction.

The $G_{3,0}$ and $G_{3,-2}$ measurements extend the tests to
third-order harmonic modes. The measured field components reproduce
the main spatial features predicted by the finite-coil COMSOL
calculation. These measurements demonstrate that the shim-coil array
can generate more complicated spatial field patterns in addition to
the lower-order modes.

The door-scan measurements provide an additional check of the
vector-field analysis. Applying a $5^\circ$ rotation of the measured
$B_x$ and $B_z$ components reduces the component mixing visible before
the correction and improves the comparison with COMSOL. This result is
...
The harmonic-mode characterization presented here is based mainly on
one-dimensional scans along the $x$ direction, supplemented by
measurements along the $y$ direction through the MSR door. These
measurements do not by themselves demonstrate the final magnetic-field
homogeneity throughout the full EDM-cell volume. The results should
therefore be viewed as a characterization and validation of the
prototype shim-coil system rather than as a final demonstration of the
TUCAN shimming requirements.

The next stage of this work will be to integrate the shim-coil system
with the $B_0$ coil and determine the shim-current settings under the
magnetic-field conditions of the assembled experiment. The field mapper
and, potentially, Cs magnetometers can be used to characterize the
residual field and determine the required shim-coil corrections.
Continued refinement of the response matrix using the measured
individual-coil responses will support this optimization.

Overall, the measurements show that the square shim-coil array can
generate controlled magnetic-field patterns inside the TUCAN MSR and
that the measured response is described reasonably well by the COMSOL
model. The results also show the importance of individual-coil
calibration, continued validation of the response matrix, and careful
treatment of the measurement coordinate system for accurate magnetic
field control.
